...
:boom:**`dfs只能是一笔画**`**，因此对于这种不能按方格dfs得情况，可以搜索切割的线

要切出对称的图形，要从中心点开始沿着线切，且每次要把当前切的线中心对称的那条线也标记为走过（走一步标记两个边），当走到边界时，即把图形切成了中心对称的图形

但最后得出的总和要除4`ans/4`因为图形中心对称，即同一个图形，顺时针旋转1次、2次、3次、4次得到的图形，都属于同一种切割方案

#### 8 包子凑数

当n个数的最大公约数不为1的时候，凑不到的个数是无数个（扩展欧几里得），输出INF。求凑不出的数可通过动态规划完全背包求解。

上限为n个数的乘积（可能的最大不能凑出的数）

#### 10 油漆面积

:boom: 可以按照涂油漆的方法做，即把矩形覆盖的格子都标记上，但要注意以下两点：

1. 标记时使用bool二维数组，可以开更大的空间（比如10000*10000）
2. 标记时注意是[x1,x2)，即左闭右开的情况，因为从1到2只有一个格子 
...
scanf()读取单个字符的时候，如果上面的语句也有scanf, 可能会读入换行符，导致读取的字符不是自己写入的字符。如果用scanf("%c",&ch)，那么程序不会忽略为了输入而按下的回车键操作，而是认为回车键是后续操作而继续响应；而`scanf(" %c", &ch)`在%c之前空格会告诉scanf忽略前面的空行，而等待第一个非空行元素读入其中。这样就能避免问题的出现



## 总结

二维数组开到8000的时候大约占用245M内存
...
今年蓝桥杯再次梦碎，看着本篇开头的To do list，也明白了一些个中缘由。

给自己设定的待办事项，基本都没有完成，可惜了一个月的准备时间。

这次打击还是比较沉重的，希望以后不再做一些打水漂的事情了，既然认定了要做好一件事，就要做好破釜沉舟的打算，思想不能有半点儿松懈，不然面对的后果，真的是出乎意料。

加油吧，一个月中也收获了不少知识，自身的算法能力感觉来讲还是有明显进步，尽管没有通过蓝桥杯的检验（😰）。大概是思维还不够缜密，思路能够想到，但却没能应对全面的情况。

以后要善于总结、回顾，以及最重要的，做一件事情的**专注**。
